...
const fileFilter = (req, file, cb) => {
  ...
  // Also allow these MIME types for JSON files
  const allowedMimeTypes = [
    "application/json",
    "text/json",
    "text/plain", // Sometimes JSON files are sent as text/plain
  ];
  ...
  if (
    ...
    (fileExtension === ".json" && allowedMimeTypes.includes(file.mimetype))
  ) {
    ...
    cb(
      new Error(
        `File type not allowed. File: ${file.originalname}, Extension: ${fileExtension}, MIME: ${file.mimetype}`
      ),
      false
    );
  }
...
module.exports = {
  uploadFields: (fields) => {
    return [upload.fields(fields), processZipFiles];
  },
  upload: upload,
};
